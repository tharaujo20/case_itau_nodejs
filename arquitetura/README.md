...
Começando com algo básico, esta opção consiste principalmente na atuação de lambdas.</br></br>
...
- Adoção de Redis como opção para cache, mantendo os dados dos clientes em fácil acesso enquanto ele estiver usando o serviço, auxiliando também na redução da sobrecarga de leitura no AuroraDb.
- O banco de dados em AuroraDb (v2 Multi-AZ) para assegurar o valor ainda próximo da proposta inicial, que era relacional.
...
Pensando em uma evolução mais consistente, esta opção consiste principalmente na atuação containers.</br></br>
...
- Adoção de Redis como opção para cache, mantendo os dados dos clientes em fácil acesso enquanto ele estiver usando o serviço, auxiliando também na redução da sobrecarga de leitura no DynamoDb.
...
- A proposta totalmente voltada para recursos serverless é prática e possível, porém com custo mais elevado, o que pode ser um ponto ofensor ao se considerar FinOps.
